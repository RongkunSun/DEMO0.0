...
label scene1_onthetrain:

    "被炒鱿鱼了。"#(音：奇怪的吉他完蛋了的音效；画：案板上的菜刀落下，旁边摆着的鱿鱼动了一下，留下一滴眼泪）
    "连夜收拾行李，坐上了回老家的火车。"
    "还在火车上的时候，"
    menu:
        "放空地看向窗外":
            "每当有其它列车在旁边驶过，车厢内的光线就会短暂染上别的颜色。啊，现在变成红色了......"
            "以前都没有留意过。"
        "回想上班的时候...":
            # （音：几声鼓点；画：穿西装，坐办公版：猫哭耗子假慈悲、黄鼠狼给鸡拜年、狐假虎威）
            "——额，还是不想了，浮现出的都是一些不好的画面。"#（压力值+1)
